...
# def DateConvertPlus(z):
#
#     Convert = datetime.strptime(SDate,'%Y-%m-%d').date()
#     result = str(Convert + timedelta(days=z))
#     return result
#
#
# def NumberOfDaysInStartdateAndEndDate ():
#     Convert1 = datetime.strptime(StartDate,'%Y-%m-%d').date()
#     Convert2 = datetime.strptime(EndDate, '%Y-%m-%d').date()
#     result = int((Convert2-Convert1).days)
#     return result
#
# def NumberOfDaysInStartdateAndEndDate0 ():
#     Convert1 = datetime.strptime(StartDate,'%Y-%m-%d').date()
#     Convert2 = datetime.strptime(EndDate, '%Y-%m-%d').date()
#     result = int((Convert2-Convert1).days/8)
#     return result
#
# print("Date format YYYY-MM-DD")
# StartDate = input("Enter the Start date :")
...
# global SDate
# SDate = StartDate
# NOFD = NumberOfDaysInStartdateAndEndDate()
# global Temp
# Temp=NOFD
# Temp2=NumberOfDaysInStartdateAndEndDate0()
# for N in range(Temp2):
#     if Temp>=8:
#      EDate = DateConvertPlus(7)
#      res = requests.get(f'https://api.nasa.gov/neo/rest/v1/feed?start_date={SDate}&end_date={EDate}&api_key={ApiKey}')
#      data = json.loads(res.text)
#      if Temp>=8:
#          k=1
#          D=0
#          while D <= 8:
#             for i in res.json()['near_earth_objects'][SDate]:
#                name_of_objects = i['name']
#                estimated_diameter_min = i['estimated_diameter']['kilometers']['estimated_diameter_min']
#                estimated_diameter_max = i['estimated_diameter']['kilometers']['estimated_diameter_max']
#                for item in i['close_approach_data']:
...
#             D+=1
#             while SDate != EndDate:
#                SDate=DateConvertPlus(k)
#                break
#          Temp -= 7
#          break
#      else:
#            EDate = DateConvertPlus(Temp)
#            res = requests.get(
#                f'https://api.nasa.gov/neo/rest/v1/feed?start_date={SDate}&end_date={EDate}&api_key={ApiKey}')
#            data = json.loads(res.text)
#            k = 1
#            D = 0
#            while D <= 8:
...
#                        miss_distance = item['miss_distance']['kilometers']
#
#                        print(name_of_objects, estimated_diameter_min, estimated_diameter_max, close_approach_data,
#                              miss_distance)
#                D += 1
#                while SDate != EndDate:
#                    SDate = DateConvertPlus(k)
